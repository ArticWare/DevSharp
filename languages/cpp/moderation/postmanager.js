...
async function acceptPost(id,content,author,writer){
  ...
  await setDoc(doc(db, "v_cpp_posts", id),data);
  await deleteDoc(doc(db, "cpp_posts", id));
  location.reload();
}
...
async function openPost(doc_id,content,author,writer){
  const doc_ref = doc(db, "cpp_posts", doc_id);
  try {
...
onAuthStateChanged(auth,async function(user) {
  if (!user) {
    ...
    admins.forEach(async (doc) => {
        if (doc.id==user.uid){
          isadmin=true;
          const q = query(collection(db,"cpp_posts"), limit(1));
